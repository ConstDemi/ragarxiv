...
# A/B на трудном golden: cross-encoder поднял recall@1 0.40→0.56 (+16 п.п.), MRR +0.095;
# BM25-гибрид навредил (кросс-язык RU→EN). Но реранкер ~1.1 ГБ VRAM рядом с 7B на 8 ГБ НЕ влезает
# → по умолчанию ВЫКЛ; включать на GPU ≥10 ГБ (или для judge-free retrieval-прогона без 7B).
RERANK_ENABLED = False
RERANK_MODEL = "BAAI/bge-reranker-v2-m3"   # мультиязычный cross-encoder (запросы RU, доки EN)
RERANK_POOL = 50                            # сколько чанков тянуть из Qdrant под реранк (потом обрезаем до RETRIEVE_K)
